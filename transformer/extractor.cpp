...
transform(PyObject * self, PyObject * args) {
    ...
    char * replacement;
    char * filename;
    if (! PyArg_ParseTuple(args, "s|s|i|i|i|i", &filename, &replacement, &start_line, &start_col, &end_line, &end_col)) {
       std::cout << "Failed to parse arguments to C extension" << std::endl;
       Py_RETURN_FALSE;
    }
	std::ifstream file(filename);
	...
	file.close();
    runToolOnCode(new ASTPatchAction(start_line, start_col, end_line, end_col, std::string(replacement), std::string(filename)), data);
    Py_RETURN_TRUE;
}
...
PyMethodDef extractor_methods[] = {
		...
		{"transform", transform, METH_VARARGS, "Transforms the target program with a replacement"},
		{NULL, NULL, 0, NULL},
};
