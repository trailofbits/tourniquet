...
public:
  explicit ASTPatchConsumer(clang::ASTContext *Context, Rewriter * rewriter,
		  int start_line, int start_col, int end_line, int end_col, std::string replacement) {
	  	  ...
	  	  SourceLocation start_loc = srcMgr.translateFileLineCol(entry, start_line, start_col);
	  	  SourceLocation end_loc = srcMgr.translateFileLineCol(entry, end_line, end_col);
...
public:
  ASTPatchAction(int start_line, int start_col, int end_line, int end_col, std::string replacement, std::string filepath) :
	  start_line(start_line), start_col(start_col), end_line(end_line), end_col(end_col), replacement(replacement), filepath(filepath) {}

  //TODO There is probably a better place to do this, HandleTranslationUnit maybe?
  ...
  std::unique_ptr<ASTConsumer> CreateASTConsumer(CompilerInstance &Compiler, StringRef file) override {
    ...
    return std::unique_ptr<clang::ASTConsumer>(
    		new ASTPatchConsumer(&Compiler.getASTContext(), &rewriter,
